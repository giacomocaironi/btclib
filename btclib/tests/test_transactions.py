...
def test_coinbase_1() -> None:
    ...
    transaction_in = TxIn.deserialize(block_1_coinbase_input_bytes)
    transaction_out = TxOut.deserialize(block_1_coinbase_output_bytes)
...
def test_wiki_transaction() -> None:
    ...
    transaction = Tx.deserialize(tx_bytes)
...
def test_invalid_tx_in() -> None:
    transaction_input = TxIn(
        prevout=OutPoint("00" * 31 + "01", 256 ** 4 - 1),
        scriptSig=Script(),
        nSequence=1,
        txinwitness=[],
    )
...
def test_invalid_tx_in2() -> None:
    transaction_input = TxIn(
        prevout=OutPoint("00" * 32, 0), scriptSig=Script(), nSequence=1, txinwitness=[]
    )
...
def test_invalid_tx_out() -> None:
    transaction_output = TxOut(nValue=-1, scriptPubKey=Script(["OP_RETURN"]))
...
def test_invalid_tx_out2() -> None:
    transaction_output = TxOut(
        nValue=2099999997690001, scriptPubKey=Script(["OP_RETURN"])
    )
...
def test_missing_tx_out() -> None:
    transaction_input = TxIn(
        prevout=OutPoint("ff" * 32, 0), scriptSig=Script(), nSequence=1, txinwitness=[]
    )
    transaction = Tx(0, 0, [transaction_input], [])
